...
/**
 * This class implements the remote methods defined by the RemoteBank interface.
 * It has a serious shortcoming, though: all account data is lost when the
 * server goes down.
 **/
public class RemoteBankServer extends UnicastRemoteObject implements RemoteBank {
	 ...
	 * returns the Account object. Otherwise, it throws an exception.
	 **/
	...
	/**
	 * Close the named account. This method is synchronized to make it thread
	 * safe, since it manipulates the accounts hashtable.
	 **/
	...
	public void deposit(String name, String password, FunnyMoney money)
		...
		synchronized (acct) {
			...
			acct.transactions.add("Deposited " + money.amount + " on "
					+ new Date());
		}
	}
	...
	public int getBalance(String name, String password) throws RemoteException,
			BankingException {
		Account acct = verify(name, password);
		synchronized (acct) {
			return acct.balance;
	...
	/**
	 * Return a Vector of strings containing the transaction history for the
	 * named account
	 **/
	...
	/**
	 * The main program that runs this RemoteBankServer. Create a
	 * RemoteBankServer object and give it a name in the registry. Read a system
	 * property to determine the name, but use "FirstRemote" as the default
	 * name. This is all that is necessary to set up the service. RMI takes care
	 * of the rest.
	 **/
